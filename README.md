...
- **Pickups** spawn every second or so. You carry up to **three items**; Select picks one and Fire uses it. The selection follows the item, not the slot: picking something up doesn't change it, and when the selected item runs out the highlight moves to the next one. Both item queues show on the HUD, so you always know what your opponent has and what they're about to use.
