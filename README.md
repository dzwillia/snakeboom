...
- **Length is storage.** You carry one item per **150 units of body** (at least one, at most six). Select picks one and Fire uses it; the selection follows the item, not the slot. Grow and the HUD's locked slots open up; get cut and the weapons that no longer fit **drop as pickups along the fallen body**, oldest first, for anyone to grab, the cutter included. Boosting past a slot boundary drops your oldest item at the tail. Both item queues show on the HUD, so you always know what your opponent has and what they're about to use.
- **Pickups** spawn every second or so and vanish after 12 s, dropped ones included.
