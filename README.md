...
- **House rules.** The admin page at `snakeboom.com/admin` (it needs the token set on the box) is the tuning panel pointed at the relay: publish it and every new online match starts with those overrides on the defaults, shown as HOUSE RULES in the lobby. Local play runs on the same house rules (the game reads them from the relay when it starts), with your own browser's tuning on top; the title says TUNED when your tuning differs from them. The admin page also shows the rules as `DEFAULT_CONFIG` lines to paste into a pull request once they feel right. ESC pauses a local match; SPACE resumes and ESC again leaves it.
- **Up to eight players.** Free-for-all rooms of 2–8 online, or a local match against up to seven bots (the PLAYERS row on the title screen). With more than two, snakes spawn on a ring facing the centre, every round scores by **finishing place** (last standing gets N−1 points down to 0; FIRST TO n means n round wins' worth), the round cap grows 10 s per extra player up to 90 s, pickups scale with the crowd, and dead players spectate (Left/Right pick a snake, Up/Down zoom). Your panel stays on the left of the HUD and everyone else is on a scoreboard to the right; heads carry name tags. A player who drops mid-match keeps their seat: their snake coasts on while the others play, and they can rejoin for the next round.
