...
- **House rules.** The admin page at `snakeboom.com/admin` (it needs the token set on the box) is the tuning panel pointed at the relay: publish it and every new online match starts with those overrides on the defaults, shown as HOUSE RULES in the lobby. Local play runs on the same house rules (the game reads them from the relay when it starts), with your own browser's tuning on top; the title shows the rules and says TUNED when your tuning differs from them. ESC pauses a local match; SPACE resumes and ESC again leaves it.
