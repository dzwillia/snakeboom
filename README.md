...
- **House rules.** The admin page at `snakeboom.com/admin` (it needs the token set on the box) is the tuning panel pointed at the relay: publish it and every new online match starts with those overrides on the defaults, shown as HOUSE RULES in the lobby. Local play uses your own browser's tuning, and the title says TUNED when that differs from the defaults.
